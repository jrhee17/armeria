...
class TBaseSerializerTest {
    ...
    private static FooStruct fooStruct() {
        return new FooStruct().setBoolVal(true)
                              .setByteVal((byte) 3)
                              .setI16Val((short) 4)
                              .setI32Val(5)
                              .setI64Val(6)
    ...
    private static OptionalFooStruct optionalFooStruct() {
    ...
    private static RequiredFooStruct requiredFooStruct() {
    ...
    void jsonProtocolComparison(TBase<?, ?> tBase) throws Exception {
        ...
        final TSerializer tSerializer = new TSerializer(new TJSONProtocol.Factory());
    ...
    void typedefJsonProtocolComparison(TBase<?, ?> tBase) throws Exception {
        ...
        if (ThriftVersionDetector.minorVersion() >= 19) {
            ...
            assertThatJson(ser).whenIgnoringPaths("12", "13", "14").isEqualTo(upstreamSer);
            assertThatJson(ser).node("12").isAbsent()
                               .node("13").isAbsent()
                               .node("14").isAbsent();
        }
    }
