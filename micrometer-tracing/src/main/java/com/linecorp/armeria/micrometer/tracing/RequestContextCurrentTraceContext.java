...
import static com.linecorp.armeria.micrometer.tracing.TraceContextUtil.setTraceContext;
import static com.linecorp.armeria.micrometer.tracing.TraceContextUtil.traceContext;
 ...
 */
public final class RequestContextCurrentTraceContext implements CurrentTraceContext {
    ...
    RequestContextCurrentTraceContext(List<Pattern> nonRequestThreadPatterns, boolean scopeDecoratorAdded) {
    ...
    public Scope newScope(@Nullable TraceContext currentSpan) {
        ...
        if (currentSpan != null && TraceContextUtil.PingPongExtra.maybeSetPong(currentSpan)) {
            return CurrentTraceContext.NOOP.newScope(currentSpan);
        }
    ...
    public <C> Callable<C> wrap(Callable<C> task) {
        final TraceContext context = context();
        return new Callable<C>() {
            @Override
            public C call() throws Exception {
                try (Scope unused = maybeScope(context)) {
                    return task.call();
                }
            }
        };
