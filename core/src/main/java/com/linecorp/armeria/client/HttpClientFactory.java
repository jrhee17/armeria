...
final class HttpClientFactory implements ClientFactory {
    ...
    HttpClientFactory(ClientFactoryOptions options, boolean autoCloseConnectionPoolListener,
        ...
        sslContextFactory = new ClientSslContextFactory(meterIdPrefix, options.meterRegistry(),
                                                        allowUnsafeCiphers);
        defaultSslContexts = new DefaultSslContexts(options, tlsNoVerifySet, insecureHosts, sslContextFactory);
    ...
    private void closeAsync(CompletableFuture<?> future) {
        ...
        CompletableFuture.allOf(dependencies.toArray(EMPTY_FUTURES)).handle((unused, cause) -> {
            ...
            defaultSslContexts.release(sslContextFactory);
    ...
    DefaultSslContexts defaultSslContexts() {
