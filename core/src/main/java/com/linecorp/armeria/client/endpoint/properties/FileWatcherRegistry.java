...
final class FileWatcherRegistry implements AutoCloseable {
    ...
    private static class FileWatchServiceContext {
        ...
        /**
         * Unregisters a {@link FileWatchRegisterKey}. On changes to the {@code path} corresponding
         * to the {@link FileWatchRegisterKey}, the {@code callback} won't be invoked anymore. If
     ...
     * file of the path of the {@code watchEventKey} is changed, then the {@code callback} function
     * is invoked. If the {@code watchEventKey} is already registered, then nothing happens.
     * This method is thread safe.
     * @param filePath path of the file to start watching for changes.
