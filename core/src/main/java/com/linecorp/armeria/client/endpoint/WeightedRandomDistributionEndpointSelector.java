...
final class WeightedRandomDistributionEndpointSelector
        extends WeightedRandomDistributionSelector<EndpointEntry> {

    WeightedRandomDistributionEndpointSelector(List<Endpoint> endpoints) {
        super(mapEndpoints(endpoints));
    }

    private static List<EndpointEntry> mapEndpoints(List<Endpoint> endpoints) {
        return endpoints.stream().map(EndpointEntry::new).collect(ImmutableList.toImmutableList());
    }

    @Nullable
    Endpoint selectEndpoint() {
        final EndpointEntry entry = select();
        if (entry == null) {
            return null;
        }
    ...
    @VisibleForTesting
    static final class EndpointEntry extends Entry {
