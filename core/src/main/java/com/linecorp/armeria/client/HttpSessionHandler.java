...
final class HttpSessionHandler extends ChannelDuplexHandler implements HttpSession {
    ...
    static final AttributeKey<Object> NESTED_EXCEPTION =
            AttributeKey.valueOf(HttpSessionHandler.class, "NESTED_EXCEPTION");
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        if (needsRetryWithH1C) {
            ...
            if (ctx.channel().hasAttr(NESTED_EXCEPTION)) {
                exception = ClosedSessionException.get(
                        (Exception) ctx.channel().attr(NESTED_EXCEPTION).get());
            } else {
                exception = ClosedSessionException.get();
            }
            failUnfinishedResponses(exception);
            ...
            sessionTimeoutFuture.cancel(false);
            sessionPromise.tryFailure(exception);
        }
    }
