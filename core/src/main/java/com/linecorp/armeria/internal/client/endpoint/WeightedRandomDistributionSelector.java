...
import com.linecorp.armeria.internal.client.endpoint.WeightedRandomDistributionSelector.Entry;
import com.linecorp.armeria.internal.common.util.ReentrantShortLock;

/**
 * This selector selects an {@link Entry} using random and the weight of the {@link Entry}. If there are
 * A(weight 10), B(weight 4) and C(weight 6) {@link Entry}s, the chances that {@link Entry}s are selected
 * are 10/20, 4/20 and 6/20, respectively. If {@link Entry} A is selected 10 times and B and C are not
 * selected as much as their weight, then A is removed temporarily and the chances that B and C are selected
 * are 4/10 and 6/10.
 */
public class WeightedRandomDistributionSelector<T extends Entry> {
