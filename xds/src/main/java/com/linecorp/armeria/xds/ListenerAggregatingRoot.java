...
public class ListenerAggregatingRoot implements
                                     XdsNode<ListenerResourceHolder, RouteConfigurationSnapshot> {

    private final WatchersStorage watchersStorage;
    private final String resourceName;
    @Nullable
    private RouteConfigurationAggregatingNode routeConfigurationAggregatingNode;
    @Nullable
    ...
    public void onChanged(ListenerResourceHolder update) {
        if (routeConfigurationAggregatingNode != null) {
            routeConfigurationAggregatingNode.close();
        }
        ...
        if (connectionManager.hasRds()) {
            ...
            routeConfigurationAggregatingNode = new RouteConfigurationAggregatingNode(watchersStorage, update, rds.getRouteConfigName(), this);
        } else if (connectionManager.hasRouteConfig()) {
            final RouteConfiguration routeConfig = connectionManager.getRouteConfig();
            routeConfigurationAggregatingNode = new RouteConfigurationAggregatingNode(watchersStorage, update, routeConfig.getName(), this);
        }
    }

    @Override
    public void newSnapshot(ListenerResourceHolder listenerHolder,
                            RouteConfigurationSnapshot routeConfigurationSnapshot) {
        listenerSnapshot = new ListenerSnapshot(listenerHolder, routeConfigurationSnapshot);
