...
final class SotwActualStream implements StreamObserver<DiscoveryResponse>, AdsXdsStream.ActualStream {
    ...
    void nackResponse(XdsType type, String nonce, String errorDetail) {
        ackBackoffAttempts++;
        noncesMap.put(type, nonce);
        eventLoop.schedule(() -> sendDiscoveryRequest(type, stateCoordinator.getVersion(type),
    ...
    private void sendDiscoveryRequest(XdsType type) {
        sendDiscoveryRequest(type, stateCoordinator.getVersion(type),
    ...
    private <I extends Message, O extends XdsResource> void handleResponse(
        ...
        if (!holder.errors().isEmpty()) {
            holder.invalidResources().forEach((name, error) ->
                                                      stateCoordinator.onResourceError(resourceParser.type(), name, error));
        ...
        stateCoordinator.onAck(resourceParser.type(), response.getVersionInfo());
