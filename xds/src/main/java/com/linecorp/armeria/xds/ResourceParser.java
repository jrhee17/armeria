...
abstract class ResourceParser<T extends Message> {
    ...
    abstract ResourceHolder<T> parse(Message message);

    ParsedResourcesHolder<T> parseResources(List<Any> resources) {
        final Map<String, ResourceHolder<T>> parsedResources = new HashMap<>(resources.size());
        ...
        for (int i = 0; i < resources.size(); i++) {
            ...
            final ResourceHolder<T> resourceUpdate;
            try {
