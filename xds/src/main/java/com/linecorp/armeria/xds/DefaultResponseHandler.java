...
final class DefaultResponseHandler implements XdsResponseHandler {
    ...
    public <T extends Message> void handleResponse(
            ResourceParser<T> resourceParser, DiscoveryResponse response, AggregatedXdsStream sender) {
        ...
        for (Map.Entry<String, XdsStreamSubscriber<?>> entry : subscribedResources.entrySet()) {
            ...
            final XdsStreamSubscriber<T> subscriber = (XdsStreamSubscriber<T>) entry.getValue();
            ...
            if (holder.parsedResources().containsKey(resourceName)) {
                ...
                subscriber.onData(holder.parsedResources().get(resourceName), response.getVersionInfo());
                continue;
            }
    ...
    public void handleReset(XdsStreamSender sender) {
        storage.allSubscribers().keySet().forEach(sender::updateResources);
    }
}
