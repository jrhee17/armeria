...
final class DeltaActualStream implements StreamObserver<DeltaDiscoveryResponse>, AdsXdsStream.ActualStream {
    ...
    // Tracks what was last sent per type on this stream; reset on each new ActualStream.
    private final Map<XdsType, Set<String>> previouslySentResources = new EnumMap<>(XdsType.class);
    ...
    void nackResponse(XdsType type, String nonce, String errorDetail) {
        enqueueAck(type, nonce, errorDetail);
    }
    ...
    private void drainRequests() {
        ...
        try {
            while (!completed) {
                ...
                if (ack != null) {
                    sendDeltaAck(ack.type, ack.nonce, ack.errorDetail);
                    continue;
                }
    ...
    private void sendDeltaRequest(XdsType type) {
        if (completed) {
            return;
        }
        final Set<String> current = new HashSet<>(stateCoordinator.interestedResources(type));
        final Set<String> previous = previouslySentResources.getOrDefault(type, Collections.emptySet());

        final Set<String> subscribe = new HashSet<>(current);
        subscribe.removeAll(previous);

        final Set<String> unsubscribe = new HashSet<>(previous);
        unsubscribe.removeAll(current);

        final DeltaDiscoveryRequest.Builder builder = DeltaDiscoveryRequest.newBuilder()
                                                                           .setTypeUrl(type.typeUrl())
                                                                           .setNode(node)
                                                                           .addAllResourceNamesSubscribe(subscribe)
                                                                           .addAllResourceNamesUnsubscribe(unsubscribe);

        // On the first request per type per stream, send what we already know so the server
        // can skip re-sending unchanged resources.
        if (!initialVersionsSent.contains(type)) {
            builder.putAllInitialResourceVersions(stateCoordinator.resourceVersions(type));
            initialVersionsSent.add(type);
        }

        previouslySentResources.put(type, new HashSet<>(current));
        final DeltaDiscoveryRequest request = builder.build();
        lifecycleObserver.requestSent(request);
        requestObserver.onNext(request);
    }

    private void sendDeltaAck(XdsType type, String nonce, @Nullable String errorDetail) {
        if (completed) {
            return;
        }
        final DeltaDiscoveryRequest.Builder builder = DeltaDiscoveryRequest.newBuilder()
    ...
    private <I extends Message, O extends XdsResource> void handleResponse(
        ...
        final String systemVersionInfo = response.getSystemVersionInfo();
        final List<Resource> deltaResources = response.getResourcesList();
        final ParsedResourcesHolder holder = resourceParser.parseDeltaResources(deltaResources, systemVersionInfo);
        ...
        stateCoordinator.onAck(type, systemVersionInfo);
