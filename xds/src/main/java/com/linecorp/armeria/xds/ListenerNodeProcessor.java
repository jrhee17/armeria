...
interface ListenerNodeProcessor extends BaseNodeProcessor {
    ...
    default void process(ListenerResourceHolder holder) {
        ...
        if (connectionManager.hasRouteConfig()) {
            ...
            children().add(watchersStorage().addStaticNode(holder, self(), XdsType.ROUTE, routeConfig.getName(), routeConfig));
            ...
            children().add(watchersStorage().subscribe(holder, self(), configSource, XdsType.ROUTE, routeName));
        } else {
