...
final class HeaderMutation implements HeaderMutator {

    static final HeaderMutation EMPTY = new HeaderMutation(Collections.emptyList(), Collections.emptyList());

    private final List<HeaderValueOption> headersToAdd;
    private final List<String> headersToRemove;

    private HeaderMutation(List<HeaderValueOption> headersToAdd, List<String> headersToRemove) {
        this.headersToAdd = headersToAdd;
        this.headersToRemove = headersToRemove;
    }

    static HeaderMutation of(List<HeaderValueOption> toAdd, List<String> toRemove) {
        if (toAdd.isEmpty() && toRemove.isEmpty()) {
            return EMPTY;
        }
        return new HeaderMutation(toAdd, toRemove);
    }
